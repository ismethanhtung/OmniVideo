...
export type VideoVipProcessingInput = {
    ...
    subtitleStyle?: VideoEditInput["subtitles"] extends { style?: infer T }
        ? T
        : never;
    sourceTitle?: string;
...
export async function runVideoVipProcessing(
    ...
    const videoBytes = await renderVipCompositeVideo({
    ...
    const finalRenderDurationMs = Date.now() - finalRenderStartedAt;

    const metadataStartedAt = Date.now();
    const metadata = await generateVietnameseVideoMetadata({
        translatedSegments: translation.translatedSegments,
