...
describe("Topbar background progress modal", () => {
    ...
    it("renders step-aware progress details with live durations", () => {
        ...
        expect(source).not.toContain("INITIAL_PROGRESS_SEGMENT_RENDER_LIMIT");
        expect(source).not.toContain("PROGRESS_SEGMENT_RENDER_BATCH_SIZE");
        expect(source).not.toContain("visibleSegments");
        expect(source).not.toContain("hiddenSegmentCount");
        expect(source).not.toContain("Show more");
        expect(source).not.toContain("Show all");
        expect(source).toContain("segments.length} segments");
