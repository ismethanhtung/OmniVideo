...
describe("WorkspaceCanvasPanel canvas interactions", () => {
    ...
    it("publishes step-aware background progress without fake step percentages", () => {
        ...
        expect(source).toContain("VIP · Upload source");
        ...
        expect(source).toContain("fetchWorkspaceJsonWithUploadProgress");
        expect(source).toContain("xhr.upload.onprogress");
        expect(source).toContain("Uploading source video to Vercel before transcript work");
        expect(source).toContain("Source upload sent to VIP API.");
        ...
        expect(source).toContain("use the cast/gender map as supporting evidence");
        expect(source).toContain("do not assume the viewer is male");
