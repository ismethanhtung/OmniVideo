...
function DetailCell({
    ...
    return (
        <div className="border border-main bg-secondary/20 px-3 py-2">
            ...
            <p
                className={`mt-1 text-[12px] text-main ${mono ? "font-mono text-[11px]" : ""}`}
            >
                {value ?? "-"}
            </p>
        </div>
...
function buildDestinationLabel({
    ...
    const account = accounts.find(
        (entry) => entry._id === destination.socialAccountId,
    );
...
export function PublishRecordsPanel({ section }: PublishRecordsPanelProps) {
    ...
    >({});
    const [selectedRecord, setSelectedRecord] = useState<PublishRecord | null>(
        null,
    );
    ...
    const selectedAssetShortHint = useMemo(() => {
        ...
        const durationMs =
            typeof selectedAsset.durationMs === "number"
        ...
        async (nextPage = 1) => {
            ...
            try {
                ...
                const accountsPayload =
                    (await accountsResponse.json()) as ApiResponse<
                        SocialAccount[]
                    >;
                const assetsPayload =
                ...
                if (!recordsResponse.ok || !recordsPayload.ok) {
                    ...
                    setMessage(
                        recordsPayload.error ??
                            "Could not load publish records.",
                    );
                    return;
                }
                ...
                setMessage(
                    `Loaded ${(recordsPayload.data ?? []).length} of ${
                        recordsPayload.pagination?.total ??
                        (recordsPayload.data ?? []).length
                    } publish records.`,
                );
            } catch (error) {
                setStatus("failed");
    ...
    const ensureFacebookPages = async (accountId: string) => {
        ...
        try {
            ...
            if (!response.ok || !payload.ok || !payload.data) {
                throw new Error(
                    payload.error ?? "Could not load Facebook pages.",
                );
            }
    ...
    const createRecord = async () => {
        ...
        if (destinations.length === 0) {
            setStatus("failed");
            setFormMessage(
                "Add at least one valid destination before publishing.",
            );
            return;
        }
        ...
        const updatePublishProgress = ({
            ...
            const nextProgress = Math.round(
                ((destinationIndex + stagePercent / 100) /
                    destinations.length) *
                    100,
            );
        ...
        try {
            for (const [index, destination] of destinations.entries()) {
                ...
                try {
                    ...
                    const payload =
                        (await response.json()) as ApiResponse<PublishRecord>;
                    updatePublishProgress({
                        destinationIndex: index,
                        stagePercent: 85,
                        destinationLabel,
                    ...
                    const isSuccess =
                        response.ok && payload.ok && Boolean(payload.data);
                    ...
                    if (isSuccess) {
                        ...
                        results.push({
                            ...
                            error:
                                payload.error ??
                                "Could not create publish record.",
                        });
                    }
                } catch (error) {
                    failedCount += 1;
            ...
            const firstError =
                failed[0]?.error ?? "Could not create publish record.";
    ...
    const deleteFailedRecords = async () => {
        if (!confirm("Delete all failed publish records?")) {
            return;
        }

        setStatus("loading");
        setMessage("Deleting failed publish records...");

        try {
            const response = await fetch(
                "/api/social/publish-records?status=failed",
                {
                    method: "DELETE",
                },
            );
            ...
            if (!response.ok || !payload.ok) {
                setStatus("failed");
                setMessage(
                    payload.error ?? "Could not delete failed publish records.",
                );
                return;
            }
            ...
            setStatus("ready");
            setMessage(
                `Deleted ${payload.data?.deletedCount ?? 0} failed publish record(s).`,
            );
        } catch (error) {
            setStatus("failed");
            setMessage(
                error instanceof Error
                    ? error.message
                    : "Could not delete failed publish records.",
            );
        }
    };
        ...
        <section className="overflow-hidden border border-main bg-main">
                        ...
                        {records.length === 0 ? (
                            ...
                            records.map((record) => {
                                const linkedAsset = assets.find(
                                    (asset) => asset._id === record.assetId,
                                );
                                const previewBlockedReason = linkedAsset
                                    ? getTelegramDownloadBlockedReason({
                                          storageProvider:
                                              linkedAsset.storageProvider,
                                          sizeBytes: linkedAsset.sizeBytes,
                                      })
                                    : "No asset preview.";
                                    ...
                                    >
                                        <td className="w-[120px] p-0">
                                            ...
                                            !previewBlockedReason ? (
                                                <a
                                                    href={`/api/storage/assets/${linkedAsset._id}/download?disposition=inline`}
                                                    target="_blank"
                                                    rel="noreferrer"
            ...
            {selectedRecord ? (
                <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/35 px-4 py-6">
                    <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto border border-main bg-main shadow-xl">
                        <div className="flex items-start justify-between gap-4 border-b border-main bg-secondary/35 px-4 py-3">
                            <div className="min-w-0">
                                <p className="truncate text-[13px] font-semibold text-main">
                                    {selectedRecord.asset?.title ??
                                        selectedRecord.title ??
                                        selectedRecord.assetId}
                                </p>
                                <p className="mt-1 text-[11px] text-muted">
                                    {formatPlatform(selectedRecord.platform)} ·{" "}
                                    {formatPublishType(
                                        selectedRecord.publishType,
                                    )}
                                </p>
                            </div>
                            <button
                        ...
                        <div className="grid gap-3 px-4 py-4 md:grid-cols-2">
                            ...
                            <DetailCell
                                label="Account"
                                value={
                                    selectedRecord.socialAccount?.label ??
                                    selectedRecord.socialAccountId
                                }
                            />
                            <DetailCell
                                label="Schedule"
                            ...
                            <DetailCell
                                ...
                                value={
                                    selectedRecord.errorCode
                                        ? `${selectedRecord.errorCode}: ${selectedRecord.errorDetail ?? ""}`
                                        : "-"
                                }
                            />
                        </div>
            ...
            <div className="mt-3 flex items-center justify-between px-5 pb-5">
                ...
                <div className="flex gap-2">
                    <button
                        type="button"
                        onClick={() =>
                            void loadAll(Math.max(1, pagination.page - 1))
                        }
                        ...
                        <div className="flex items-start justify-between gap-3 border-b border-main bg-secondary/35 px-4 py-3">
                            <div>
                                ...
                                <p className="mt-1 text-[11px] leading-5 text-muted">
                                    Shopee hiện vẫn deferred nên publish now sẽ
                                    fail rõ ràng.
                                </p>
                            </div>
                            <button
                            ...
                            {isSubmitting ? (
                                <div className="border border-main bg-secondary/20 p-3 md:col-span-2">
                                    ...
                                    <p className="mt-2 text-[11px] leading-5 text-muted">
                                        Bạn có thể bấm Hide; tiến trình vẫn nằm
                                        trong nút Progress trên topbar.
                                    </p>
                                </div>
                            ) : null}
                                ...
                                <div className="mt-2 grid gap-2">
                                    <label className="flex items-start gap-2 text-[12px] text-main">
                                        ...
                                        <span>
                                            ...
                                            <span className="block text-[11px] text-muted">
                                                Upload ngay cho
                                                YouTube/TikTok/Facebook. Các
                                                platform chưa có adapter thật sẽ
                                                trả lỗi chưa hỗ trợ.
                                            </span>
                                        </span>
                                    </label>
                                    <label className="flex items-start gap-2 text-[12px] text-main">
                                        ...
                                        <span>
                                            Schedule / plan
                                            <span className="block text-[11px] text-muted">
                                                Keep this as a planned record
                                                for a selected time or later
                                                review.
                                            </span>
                                        </span>
                                    </label>
                                        ...
                                        {form.destinations.map(
                                            (destination, index) => {
                                                ...
                                                return (
                                                    <div
                                                        key={destination.id}
                                                        className="border border-main bg-main p-3"
                                                    >
                                                        ...
                                                        <div className="grid gap-3 md:grid-cols-2">
                                                            ...
                                                            <label className="text-[12px] font-medium text-main">
                                                                ...
                                                                >
                                                                    ...
                                                                    {(
                                                                        destinationAccount?.supportedFormats ??
                                                                        []
                                                                    ).map(
                                                                        (
                                                                            format,
